...
- **必要な画面や機能**: システムが提供すべき機能の一覧、画面の概要、入出力項目を定義します。機能要件はユーザー操作や業務フローに基づき、具体的かつ実現可能な範囲で詳細化します。
...
1. **コンセプト定義フェーズ**: コンセプト定義書を作成します。
1. **要件定義フェーズ**: 要件定義書を作成します。重要なアーキテクチャ上の決定が発生した場合は、ADRを作成します。
1. **設計フェーズ**: 基本設計書、詳細設計書を作成します。重要なアーキテクチャ上の決定が発生した場合は、ADRを作成します。
1. **実装フェーズ**: 必要に応じてプログラム設計書やコードコメントなどを記述します。
1. **テストフェーズ**: テスト仕様書を作成し、テスト実施後にテスト報告書を作成します。
1. **運用・保守フェーズ**: 運用マニュアル、操作説明書などを作成します。
