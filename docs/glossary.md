...
**本用語集に記載されている一部のプロセス、メソドロジー、コンセプトの成功要因および失敗要因については、`docs/concept/researches/it-project-challenge-solutions-report.md`
を参照してください。**
...
| ケバブケース (kebab-case)
| ファイル名やディレクトリ名に使用される命名規約の一つで、単語をハイフン (`-`) で繋ぐ形式。 |
| | 共有されたオーナーシップとドメイン知識の構築 |
GTDをチームで利用する背景・理由の一つ。チームが一貫したシステムを使用することで、メンバーが共通の理解を深め、誤解のリスクを減らす。 |
GTD | | 生産性プラクティスのスケーラビリティ |
GTDをチームで利用する背景・理由の一つ。個人のGTDメリットをチーム全体にスケールアップし、組織全体の目標達成に貢献する。 |
GTD | | ストレスと認知負荷の軽減 |
GTDをチームで利用するメリットの一つ。コミットメントを外部システムに置くことで、チームメンバーの精神的な負担を軽減する。 |
GTD | | フィーチャーブランチワークフロー |
Gitのブランチ戦略の一つで、新機能開発やバグ修正を`main`ブランチから派生したフィーチャーブランチで行う。 |
|
| アウトプット中心タスク管理 | タスクを完了させることよりも、具体的な成果物（アウトプット）を生み出すことに焦点を当てたタスク管理手法。 |
|
| アウトプット中心思考 | タスクやプロジェクトを進める際に、最終的な成果物や目標を明確に意識し、そこから逆算して計画を立てる思考法。 |
GTD | | GTD概要 | GTD（Getting Things
Done）の基本的な概念、原則、およびワークフローの全体像をまとめたドキュメント。 |
GTD | | GTDチーム導入のメリット |
GTDを個人だけでなくチーム全体で導入することによって得られる具体的な利点や効果。 |
GTD |
